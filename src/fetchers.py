    ...
    def fetch(self, url: str, use_robots: bool = True) -> Optional[FetchResult]:
        ...
        if self.use_cache:
            ...
            if age is not None and age < 24:
                return FetchResult(url=canon, status_code=200, text=load_text(cache_path), final_url=canon, from_cache=True)

        try:
            resp = self.session.get(canon, timeout=self.timeout)
            text = resp.text if resp.status_code < 400 else ""
            final_url = canonicalize_url(resp.url or canon)
            if self.use_cache and text:
                save_text(cache_path, text)
            return FetchResult(url=canon, status_code=resp.status_code, text=text, final_url=final_url, from_cache=False)
...
def discover_homepage_urls(fetcher: WebFetcher) -> Dict[str, Set[str]]:
    found: Dict[str, Set[str]] = {"Music Week": set(), "Music Business Worldwide": set()}
    homepage = "https://www.musicweek.com/"
    res = fetcher.fetch(homepage, use_robots=True)
...
def discover_fallback_urls(fetcher: WebFetcher) -> Dict[str, Set[str]]:
    ...
    # MBW RSS-only discovery (no robots checks, no web fetch).
    ...
    for idx, list_url in enumerate(listing_urls, start=1):
        res = fetcher.fetch(list_url, use_robots=True)
        if not res:
            continue
        if idx > 1 and res.status_code == 404:
            logging.info("Music Week pagination unavailable (404): %s; stopping pagination.", list_url)
            break
...
def fetch_mbw_rss_articles(days: int) -> List[Dict]:
    ...
    for entry in getattr(feed, "entries", []):
        ...
        excerpt = truncate_text(strip_html((getattr(entry, "summary", "") or "").strip()), 300)

        entries.append(
            {
                "source": "Music Business Worldwide",
                "date": published_raw if published_raw else "",
