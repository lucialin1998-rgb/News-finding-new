...
def discover_homepage_urls(fetcher: WebFetcher) -> Dict[str, Set[str]]:
    homepages = {
        ...
        # MBW runs in RSS-only mode for discovery to avoid robots/homepage blocking.
        "Music Business Worldwide": "",
    }
    found: Dict[str, Set[str]] = {"Music Week": set(), "Music Business Worldwide": set()}

    homepage = homepages["Music Week"]
...
def discover_fallback_urls(fetcher: WebFetcher) -> Dict[str, Set[str]]:
    ...
    # MBW RSS fallback (no robots gating).
    ...
    # Music Week listing fallback with query paging; stop on first 404 after page 1.
    listing_urls = [
